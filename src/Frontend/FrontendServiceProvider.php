...
use Bestkit\Frontend\Driver\BasicTitleDriver;
use Bestkit\Frontend\Driver\TitleDriverInterface;
...
class FrontendServiceProvider extends AbstractServiceProvider
{
    ...
    {
        $this->container->singleton('bestkit.assets.factory', function (Container $container) {
            return function (string $name) use ($container) {
                $paths = $container[Paths::class];

                $assets = new Assets(
                    $name,
                    $container->make('filesystem')->disk('bestkit-assets'),
                    $paths->storage,
                    null,
                    $container->make('bestkit.frontend.custom_less_functions')
                );
        ...
        $this->container->singleton('bestkit.frontend.factory', function (Container $container) {
            return function (string $name) use ($container) {
                $frontend = $container->make(Frontend::class);

                $frontend->content(function (Document $document) use ($name) {
                    $document->layoutView = 'bestkit::frontend.'.$name;
                });
                ...
                $frontend->content(function (Document $document) use ($container) {
                    $default_preloads = $container->make('bestkit.frontend.default_preloads');
        ...
        $this->container->singleton(
            'bestkit.frontend.default_preloads',
            function (Container $container) {
                $filesystem = $container->make('filesystem')->disk('bestkit-assets');
        ...
        $this->container->singleton(
            'bestkit.frontend.custom_less_functions',
            function (Container $container) {
        ...
        $this->container->alias(TitleDriverInterface::class, 'bestkit.frontend.title_driver');

        $this->container->singleton('bestkit.less.config', function (Container $container) {
            return [
    ...
    {
        $this->loadViewsFrom(__DIR__.'/../../views', 'bestkit');
    ...
    private function addLessVariables(SourceCollector $sources)
    {
        $sources->addString(function () {
            $vars = $this->container->make('bestkit.less.config');
            $extDefinedVars = $this->container->make('bestkit.less.custom_variables');
