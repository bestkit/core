...
class ThrottleApi implements ExtenderInterface
{
     ...
     *               `\Bestkit\Http\RequestUtil::getActor($request)` can be used to get the current user.
    ...
    {
        $container->extend('bestkit.api.throttlers', function ($throttlers) use ($container) {
