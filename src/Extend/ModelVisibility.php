...
class ModelVisibility implements ExtenderInterface
{
     ...
     *                           This model must extend from \Bestkit\Database\AbstractModel,
     *                           and use \Bestkit\Database\ScopeVisibilityTrait.
     */
    public function __construct(string $modelClass)
    {
        ...
        if (class_exists($this->modelClass) && ! is_callable([$modelClass, 'registerVisibilityScoper'])) {
            throw new Exception("Model $modelClass cannot be visibility scoped as it does not use Bestkit\Database\ScopeVisibilityTrait.");
        }
    }
