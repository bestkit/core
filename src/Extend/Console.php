...
class Console implements ExtenderInterface
{
     ...
     *
     * @param string $command: ::class attribute of command class, which must extend Bestkit\Console\AbstractCommand.
     * @return self
     */
    ...
    {
        $container->extend('bestkit.console.commands', function ($existingCommands) {
            return array_merge($existingCommands, $this->addCommands);
        });

        $container->extend('bestkit.console.scheduled', function ($existingScheduled) use ($container) {
