...
class Theme implements ExtenderInterface
{
     ...
     * @param string|null $extensionId : If overriding an extension file, specify its ID, for example: `bestkit-tags`.
     * @return self
     */
     ...
     * ```php
     * (new Extend\Theme)
     *     ->addCustomLessFunction('is-bestkit', function (mixed $text) {
     *         return strtolower($text) === 'bestkit'
     *     }),
     * ```
     *
    ...
    {
        $container->extend('bestkit.frontend.custom_less_functions', function (array $customFunctions) {
            return array_merge($customFunctions, $this->customFunctions);
        });

        $container->extend('bestkit.less.custom_variables', function (array $lessVariables) {
            return array_merge($this->lessVariables, $lessVariables);
        });

        $container->extend('bestkit.assets.factory', function (callable $factory) {
