...
class LanguagePack implements ExtenderInterface, LifecycleInterface
{
    ...
    public function extend(Container $container, Extension $extension = null)
    {
        ...
        $locale = $extension->composerJsonAttribute('extra.bestkit-locale.code');
        $title = $extension->composerJsonAttribute('extra.bestkit-locale.title');

        if (! isset($locale, $title)) {
            throw new RuntimeException(
                'Language packs must define "extra.bestkit-locale.code" and "extra.bestkit-locale.title" in composer.json.'
            );
        }
    ...
    {
        $container->make('bestkit.locales')->clearCache();
    }
