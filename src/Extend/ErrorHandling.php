...
class ErrorHandling implements ExtenderInterface
{
     ...
     * itself (if it implements {@see \Bestkit\Foundation\KnownError}), or
     ...
     * the exception implement our {@see \Bestkit\Foundation\KnownError}
     ...
     *
     * When Bestkit's default error handling is not enough for you, and the other
     ...
     * returns a {@see \Bestkit\Foundation\ErrorHandling\HandledError} instance.
     ...
     *
     * Reporters will be called whenever Bestkit encounters an exception that it
     ...
     * {@see \Bestkit\Foundation\ErrorHandling\Reporter} interface.
     *
    ...
    public function extend(Container $container, Extension $extension = null)
    {
        if (count($this->statuses)) {
            $container->extend('bestkit.error.statuses', function ($statuses) {
                return array_merge($statuses, $this->statuses);
            });
        }

        if (count($this->types)) {
            $container->extend('bestkit.error.classes', function ($types) {
                return array_merge($types, $this->types);
            });
        }

        if (count($this->handlers)) {
            $container->extend('bestkit.error.handlers', function ($handlers) {
