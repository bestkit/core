...
use Bestkit\Frontend\Frontend as ActualFrontend;
...
class Frontend implements ExtenderInterface
{
     ...
     * - \Bestkit\Frontend\Document $document
    ...
    private function registerAssets(Container $container, string $moduleName): void
    {
        ...
        $abstract = 'bestkit.assets.'.$this->frontend;
            ...
            if ($this->js) {
                $assets->js(function (SourceCollector $sources) use ($moduleName) {
                    ...
                    $sources->addString(function () use ($moduleName) {
                        return "bestkit.extensions['$moduleName']=module.exports;";
                    });
                });
            }
        ...
        if (! $container->bound($abstract)) {
            $container->bind($abstract, function (Container $container) {
                return $container->make('bestkit.assets.factory')($this->frontend);
            });
    ...
    private function registerContent(Container $container): void
    {
        ...
        $container->resolving(
            "bestkit.frontend.$this->frontend",
    ...
    {
        if ($this->titleDriver) {
            $container->extend('bestkit.frontend.title_driver', function ($driver, Container $container) {
