...
use Bestkit\Extension\Event\Enabling;
use Bestkit\Extension\Event\Uninstalled;
use Bestkit\Extension\Exception\CircularDependenciesException;
...
class ExtensionManager
{
    ...
    public function getExtensions()
    {
        if (is_null($this->extensions) && $this->filesystem->exists($this->paths->vendor.'/composer/installed.json')) {
            ...
            // We calculate and store a set of composer package names for all installed Bestkit extensions,
            // so we know what is and isn't a bestkit extension in `calculateDependencies`.
            ...
            foreach ($installed as $package) {
                ...
                if (Arr::get($package, 'type') === 'bestkit-extension') {
                    $composerJsonConfs[$packagePath] = $package;
                }

                if ($subextPaths = Arr::get($package, 'extra.bestkit-subextensions', [])) {
                    foreach ($subextPaths as $subExtPath) {
                        ...
                        if (Arr::get($conf, 'type') === 'bestkit-extension') {
     ...
     * This is resolved dynamically because Bestkit's filesystem configuration
     * might not be booted yet when the ExtensionManager singleton initializes.
     */
    protected function getAssetsFilesystem(): Cloud
    {
        return resolve('filesystem')->disk('bestkit-assets');
    }
    ...
    /**
     * Call on all enabled extensions to extend the Bestkit application.
     *
     ...
     *      'valid' points to an ordered array of \Bestkit\Extension\Extension
