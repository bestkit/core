...
use Bestkit\Foundation\ErrorHandling\LogReporter;
use Bestkit\Foundation\SiteInterface;
...
class Server
{
    ...
    /**
     * Try to boot Bestkit, and retrieve the app's HTTP request handler.
     *
    ...
    private function cleanBootExceptionLog(Throwable $error)
    {
        ...
        if ($container->has('bestkit.config') && resolve('bestkit.config')->inDebugMode()) {
            ...
            echo <<<ERROR
            Bestkit encountered a boot error ($type)<br />
            <b>$message</b><br />
...
ERROR;
            ...
            echo 'Bestkit encountered a boot error. Details have been logged to the Bestkit log file.';
            exit(1);
        }
    }
    ...
    {
        echo 'Bestkit encountered a boot error. Details have been logged to the system PHP log file.<br />';
