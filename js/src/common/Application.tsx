...
import RequestError, { InternalBestkitRequestOptions } from './utils/RequestError';
...
export type BestkitScreens = 'phone' | 'tablet' | 'desktop' | 'desktop-hd';

export type BestkitGenericRoute = RouteItem<any, any, any>;

export interface BestkitRequestOptions<ResponseType> extends Omit<Mithril.RequestOptions<ResponseType>, 'extract'> {
...
> = {
   ...
   *
   * @see https://docs.bestkit.cn/extend/frontend-pages.html#route-resolvers-advanced
   */
  path: `/${string}`;
} & (
  | {
...
export default class Application {
   ...
   */
  routes: Record<string, BestkitGenericRoute> = {};
    ...
    this.initializers.toArray().forEach((initializer) => {
      try {
        ...
        const extension = initializer.itemName.includes('/')
          ? initializer.itemName.replace(/(\/bestkit-ext-)|(\/bestkit-)/g, '-')
   ...
   */
  screen(): BestkitScreens {
    const styles = getComputedStyle(document.documentElement);
    return styles.getPropertyValue('--bestkit-screen') as ReturnType<Application['screen']>;
  }
  ...
  protected transformRequestOptions<ResponseType>(bestkitOptions: BestkitRequestOptions<ResponseType>): InternalBestkitRequestOptions<ResponseType> {
    const { background, deserialize, extract, modifyText, ...tmpOptions } = { ...bestkitOptions };
    ...
    const options: InternalBestkitRequestOptions<ResponseType> = {
   ...
   */
  request<ResponseType>(originalOptions: BestkitRequestOptions<ResponseType>): Promise<ResponseType> {
   ...
   */
  protected requestErrorCatch<ResponseType>(error: RequestError, customErrorHandler: BestkitRequestOptions<ResponseType>['errorHandler']) {
   ...
   * While not certain, a failing cross-origin request likely indicates a missing redirect to Bestkit canonical URL.
